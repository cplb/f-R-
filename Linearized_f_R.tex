...
\begin{abstract}
We investigate the linearized form of metric $f(R)$-gravity, assuming that $f(R)$ is analytic about $R = 0$ so it may be expanded as $f(R) = R + a_2R^2/2 + \ldots$. We show how gravitational radiation is modified in $f(R)$-gravity, admitting an extra mode of oscillation, that of the Ricci scalar, in addition to two transverse polarizations which are similar to their general relativistic counterparts. The energy-momentum pseudotensor for the gravitational radiation is derived. We go on to derive weak-field metrics for a point source, a slowly rotating point source and a uniform density sphere, recovering results known for quadratic theories of gravities. The metrics for the first two sources are distinct from the appropriate limits of Schwarzschild and Kerr demonstrating that black holes are different in $f(R)$-gravity. The final metric shows that Birkhoff's theorem no longer applies in $f(R)$-gravity. Using these weak-field metrics we investigate various tests that could constrain $f(R)$. We find that time delay or light deflection experiments cannot distinguish $f(R)$-gravity from general relativity as the former has an effective post-Newtonian parameter $\gamma = 1$. Considering planetary precession, we find that the precession rate is enhanced relative to general relativity, and from the orbit of Mercury we derive the bound $|a_2| \lesssim 1.2 \times 10^{18}\units{m^2}$. E\"ot-Wash experiments provide a much stricter bound $|a_2| \lesssim 2 \times 10^{-9}\units{m^2}$. Although the former bound is much weaker, it is still of interest in the case that the effective form of $f(R)$ is modified in different regions, perhaps through the chameleon mechanism which reduces the effective value of $|a_2|$ in the presence of matter. If the laboratory bound is universal we conclude that the propagating Ricci scalar mode will not be excited by astrophysical sources as it has a cutoff frequency of $\gtrsim 4 \times 10^{12}\units{s^{-1}}$.
\end{abstract}
