...
L_z & = & \widetilde{r}^{\,2}\sin^2\theta\, \dot{\phi} - \frac{2GJ}{\widetilde{r}} \sin^2\theta\dot{t}b.
\end{eqnarray}
...
\end{equation}
For a circular equatorial orbit, setting $\mu = \widetilde{r}$ gives the frequency of the orbit $\omega_0 = \dd\phi/\dd t$ as
\begin{equation}
\omega_0 = -\frac{GJ}{\widetilde{r}^{\,3}} \pm \frac{1}{2} \sqrt{\frac{2A'}{\widetilde{r}} + \left(\frac{2GJ}{\widetilde{r}^{\,3}}\right)^2}
\label{eq:omz}
\end{equation}
in which a dash denotes $\dd/\dd\widetilde{r}$ and the $+/-$ sign denotes prograde/retrograde orbits. The definition of proper time then gives
\begin{equation}
...
\end{equation}
where $\overline{r}$ is the radius of the unperturbed orbit. If the perturbation is small the orbit undergoes small oscillations with frequency $\Omega_r^2 = -(1/2) (\partial^2 V/\partial \widetilde{r}^{\,2})|_{\overline{r},\theta=\pi/2}$. Small vertical perturbations $\theta=\pi/2 + \delta$ oscillate with frequency $\Omega_v^2 = -(1/2) (B(\overline{r})/\overline{r}^2) (\partial^2 V/\partial \widetilde{\theta}^2)|_{\overline{r},\theta=\pi/2}$.
...
\subsubsection{Planetary precessions}

We can also apply the precession formula to the classic test of planetary precession in the solar system. Taking the non-rotating limit, the epicyclic frequency is
\begin{equation}
\Omega^2 = \omega_0^2 \left[1 - \frac{3r\sub{S}}{\overline{r}} - \zeta(\Upsilon,r\sub{S},\overline{r})\right],
\end{equation}
defining the function
