...
U(r) = \frac{GM}{r}.
\end{equation}
The $f(R)$ metric \eqnref{f(R)_Schw} is of a similar form, but there is not a direct correspondence because of the exponential.\footnote{Our $f(R)$ theory is equivalent to a Brans-Dicke theory with a potential and parameter $\omega\sub{BD} = 0$~\cite{Teyssandier1983, Wands1994}. We cannot use the familiar result $\gamma = (1 + \omega\sub{BD})/(2 + \omega\sub{BD})$~\cite{Will2006} as this was dervied for Brans-Dicke theory without a potential~\cite{Will1998}.} It has been suggested that this may be incorporated by changing the definition of the potential $U$~\cite{Olmo2007c, Faulkner2007, Bisabr2010, DeFelice2010}, then
\begin{equation}
...
\begin{equation}
\dot{t}=\left(A + \frac{4GJ\omega_0}{\widetilde{r}} - \widetilde{r}^{\,2}\omega_0 \right)^{-1/2}
\end{equation}
from which we get $\dot{\phi} = \omega_0 \dot{t}$ and hence we have $\omega_0$, $E$ and $L_z$ in terms of $\widetilde{r}$. 
