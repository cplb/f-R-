...
\begin{abstract}
We investigate the linearized form of metric $f(R)$-gravity, assuming that $f(R)$ is analytic about $R = 0$ so it may be expanded as $f(R) = R + a_2R^2/2 + \ldots$. We show how gravitational radiation is modified in $f(R)$-gravity, admitting an extra mode of oscillation, that of the Ricci scalar, in addition to two transverse polarizations which are similar to their general relativistic counterparts. The energy-momentum pseudotensor for the gravitational radiation is derived. We go on to derive weak-field metrics for a point source, a slowly rotating point source and a uniform density sphere, recovering results known for quadratic theories of gravities. The metrics for the first two sources are distinct from the appropriate limits of Schwarzschild and Kerr demonstrating that black holes are different in $f(R)$-gravity. The final metric shows that Birkhoff's theorem no longer applies in $f(R)$-gravity. Using these weak-field metrics we investigate various tests that could constrain $f(R)$. We find that time delay or light deflection experiements cannot distinguish $f(R)$-gravity from general relativity as the former has an effective post-Newtonian parameter $\gamma = 1$. Considering planetrary precession, we find that the precession rate is enhanced relative to general relativity, and from the orbit of Mercury we derive the bound $|a_2| \lesssim 1.2 \times 10^{18}\units{m^2}$. E\"ot-Wash experiements provide a much stricter bound $|a_2| \lesssim 2 \times 10^{-9}\units{m^2}$. Although the former bound is much weaker, it is still of interest in the case that the effective form of $f(R)$ is modified in different regions, perhaps through the chameleon mechanism which reduces the effective value of $|a_2|$ in the presense of matter. If the laboratory bound is universal we conclude that the propagating Ricci scalr mode will not be excited by astrophysical sources as it has a cutoff frequency of $\gtrsim 4 \times 10^{12}\units{s^{-1}}$.
\end{abstract}
...
In this work we will look at metric $f(R)$-gravity. We focus on the modifications to gravitational radiation and possible solar system tests that can be used to constrain the theory. We begin with a review of review of the $f(R)$ field equations. Then in \secref{Lin} we derive the linearized equations. In \secref{Rad} we apply these to find wave solutions and in \secref{EM_tensor} we derive the energy-momentum pseudotensor for this gravitational radiation. In \secref{Source} we look at the effects of introducing a source term and derive the weak-field metrics for a point source, a slowly rotating point source and a uniform density sphere. These are used in \secref{Tests} derive constraints for $f(R)$ based upon solar system and laboratory tests. Some results known in the literature are worked out here {\it ab initio}; they are included as a compendium of useful results, within a consistent system of notation, and to highlight some important points.
...
\end{equation}
Including the function $f(R)$ gives extra freedom in defining the behaviour of gravity; while this action may not encode the true theory of gravity it could contain sufficient information to act as an effective field theory, correctly describing phenomenological behaviour~\cite{Park2010}. We will assume that $f(R)$ is analytic about $R = 0$ so that it can be expressed as a power series~\cite{Buchdahl1970, Clifton2008, Psaltis2008}
\begin{equation}
...
{\mathcal{G}^{(1)}}_{\mu\nu} & = & \frac{2 - b}{6}\mathcal{G}^{(1)}\eta_{\mu\nu} -\frac{1}{2}\Box\left(\overline{h}_{\mu\nu} \nonumber \\*
 & & - {} \frac{\overline{h}}{2}\right) - (b + 1)\left(a_2\partial_\mu\partial_\nu R^{(1)} + \reip{6}R^{(1)}}\eta_{\mu\nu}\right).
\label{eq:b_Field}
\end{eqnarray}
Picking $b = -1$ the final term vanishes, thus we will set~\cite{Corda2007, Capozziello2008}
\begin{eqnarray}
...
In the next section we will use these weak-field metrics with results from classic experimental tests of gravity to place constrints on $f(R)$.
